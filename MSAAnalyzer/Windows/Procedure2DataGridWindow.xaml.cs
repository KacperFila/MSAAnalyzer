...
namespace MSAAnalyzer.Windows
{
    ...
    public partial class Procedure2DataGridWindow : Window
    {
                    ...
                    foreach (var item in temp2.Children)
                    {
                        ...
                        foreach (var item2 in scroll.Children)
                        {
                            ...
                            foreach (var item3 in dataGrid.ItemsSource)
                            {
                                ...
                                {
                                    double.TryParse(dataGridItem.Value,
                                        out double value);
                                    int.TryParse(dataGridItem.OperatorKey,
                                        out int key1);
                                    int.TryParse(dataGridItem.SeriaKey,
                                        out int key2);
                                    int.TryParse(dataGridItem.WyrobKey,
                                        out int key3);

                                    _appDataContext
                                        .SecondProcedureMeasurements[(key1, key2, key3)] = value;
                                }
                                else
                                {
                                    MessageBox.Show("Wprowadzone " +
                                                    "wartości zawierają niedozwoloną wartość",
                                        "Błąd wartości", MessageBoxButton.OK,
        ...
        private bool IsValidSecondProcedureGridItem
            (SecondProcedureDataGridItem dataGridItem)
        {
